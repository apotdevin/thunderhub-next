...
const AccountProvider = ({ children }: any) => {
  ...
  const setAccount = ({ loggedIn, name, host, admin, sessionAdmin, viewOnly, cert, id }: ChangeProps) => {
  ...
  const deleteAccount = (deleteId: string) => {
    ...
    if (isCurrentAccount) {
      ...
      const newIndex = changedAccounts.findIndex((account: any) => account.id === settings.id);
